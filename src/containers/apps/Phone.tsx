...
import { IoBackspaceOutline, IoChatbubbleEllipses, IoCheckmarkCircleOutline, IoClose } from 'react-icons/io5';
...
import { AiOutlineExclamation, AiOutlinePlus, AiOutlineUser } from 'react-icons/ai';
...
export default function Phone(){
        ...
        return (
            <>
                {isEmergency ? (
                    ...
                    <div className={`relative overflow-hidden rounded-full flex justify-center items-center text-gray-100 ${image === null && color}`} style={{ width: size, height: size, fontSize: fontSize }}>
                        {image === null ? (
                            <>
    ...
    return (
        <>
            ...
            <div className={twMerge('absolute top-0 bottom-0 left-0 right-0 z-10 m-auto w-[90%] h-[90%] flex flex-col transition-all duration-300 opacity-0 pointer-events-none', phone.number !== '' && 'w-full h-full opacity-100 pointer-events-auto')}>
                        ...
                        <div className='flex flex-col bg-gradient-to-b from-[#680000]/80 to-[#a50000] w-full h-full py-8 px-4 text-gray-100'>
                            <div className="flex justify-center items-center h-80">
                                <div className='text-center'>
                                    ...
                                    {!callEnded ? <p className='text-sm'>Calling Emergency Service...</p> : <p className='text-sm'>Call Ended</p>}
                                </div>
                            </div>
                            ...
                            <div className="flex justify-center items-center h-80">
                                <div className='text-center'>
                                    {phone.number !== '' && (
                                        <>
                                            <p className='text-[42px] font-semibold mb-2'>{phone.name ? phone.name : phone.number}</p>
                                            {!callEnded ? <p className='text-sm'>Calling...</p> : <p className='text-sm'>Call Ended</p>}
                                        </>
                                    )}
                                </div>
                            </div>
