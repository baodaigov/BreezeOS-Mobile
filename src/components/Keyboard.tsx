...
export default function Keyboard({ theme = "dark" }: KeyboardProps) {
  ...
  function keyClick(key: string) {
    ...
    if (k === "emoji")
      return setTimeout(() => dispatch(setEmojiKeyboard(true)), 150);
  ...
  function generate(key: string) {
    switch (key) {
      ...
      case "space":
        return (
          <Key className="w-[198px] text-xs" onClick={() => keyClick(key)}>
            <p>{currentLang}</p>
          </Key>
        );
      case "return":
  ...
  const Key: React.FC<KeyProps> = ({
    ...
    return (
      ...
      >
        <div
          className={twMerge(
            `relative flex w-[38px] items-center justify-center ${
              theme === "dark"
                ? "bg-gray-800/30 text-gray-100 active:bg-gray-700/40"
                : "bg-gray-100/30 text-gray-800 active:bg-gray-400/10"
            } m-[3px] rounded-md px-1 py-3 transition-all duration-300 active:-translate-y-1 active:transition-none`,
            className,
          )}
        >
          {children}
  ...
  return (
    <div
      className={twMerge(
        `absolute -bottom-full z-10 ${
          theme === "dark" ? "bg-gray-900/90" : "bg-gray-50/80"
        } pointer-events-none flex w-full flex-col p-1 pb-12 opacity-0 backdrop-blur transition-all duration-300`,
    ...
    >
      <div className="flex flex-row-reverse px-2 py-1">
        <ActionButton
          className={`bg-transparent p-1 ${
            theme === "dark"
              ? "text-gray-100 active:bg-gray-100/5"
      ...
      {keyboard.emoji && (
        <div
          className={`flex h-full flex-col justify-between px-1 ${
        ...
        >
          ...
          <div className="flex">
            <ActionButton
              ...
              onClick={() => dispatch(setEmojiKeyboard(false))}
            >
